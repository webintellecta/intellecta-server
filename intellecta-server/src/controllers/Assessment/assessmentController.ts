...
import { evaluateAssessmentService, getAssessmentQuesService } from "../../service/assessmentQuesService";
...
export const getAssessmentQuestions = async (req: AuthRequest, res: Response ): Promise<void>  => {
  ...
  const { userId, age, level, questions } = await getAssessmentQuesService(req.user?.userId);
